...
export function getScoreInfo() {
  const emptyInfo: Array<Dto.ExamData> = Array();
  const userInfo: Array<Dto.ExamData> =
    getLocal<Array<Dto.ExamData>>(EnumStorageKey['exam-scores']) || emptyInfo;
  return userInfo;
}
export function setScoreInfo(userInfo: Array<Dto.ExamData>) {
  setLocal(EnumStorageKey['exam-scores'], userInfo);
}
/** 获取用户信息 */
...
export function removeExamInfo() {
  removeLocal(EnumStorageKey['exam-scores']);
}
